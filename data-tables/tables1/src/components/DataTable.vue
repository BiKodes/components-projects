<template>
    <div>
        <div class="between:flex bottom:margin-3">
            ...
            <div class="end:flex">
                <input type="search" class="input px:width-25" placeholder="Search here..." 
    ...
    <table class="table table:border secondary-5:border">
        <thead>
                ...
                <th v-for="th in tableHeader" :key="th" >
                    <div class="between:flex center:items">
                        ...
                        <span @click.prevent="sortByColumn(th.name)">
                            ...
                            fill="currentColor" class="bi bi-filter none:event" viewBox="0 0 16 16">
                                <path d="M6 10.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 0 1h-3a.5.5 0 0 
                                1-.5-.5zm-2-3a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 
                                1-.5-.5zm-2-3a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11a.5.5 0 0 
                                1-.5-.5z"/>
                            </svg>           
                        </span>
                    </div>
            ...
            <tr>
                <td>#</td>
                <td><input type="search" class="input" placeholder="Filter name ..." v-model="col.name"
                    @keyup="filterByColumn"></td>
                <td>
                    <select class="select" v-model="col.position" @change="filterByColumn">
                        <option value="">-- Filter Position --</option>
                        <option v-for="cd in uniqColumnData('position')" :key="cd" :value="cd">
                            {{ cd }}
                        </option>
                    </select>
                </td>
                <td>
                    <select class="select" v-model="col.office" @change="filterByColumn">
                        ...
                        <option v-for="cd in uniqColumnData('office')" :key="cd" :value="cd">
                            {{ cd }}
                        </option>
                    </select>
                </td>
                
                <td><input type="search" class="input" placeholder="Filter exten ..." v-model="col.extension"
                @keyup="filterByColumn"></td>
                <td><input type="search" class="input" placeholder="Filter date ..." v-model="col.startdate"
                @keyup="filterByColumn"></td>
                <td><input type="search" class="input" placeholder="Filter salary ..." v-model="col.salary"
                @keyup="filterByColumn"></td>
            </tr> 
        </thead>
        <tbody>
            ...
            <div class="end:flex">
                <ul class="pagination:nav">
                    <li :class="['nav-item', {'disabled': currentPage === 1}]">
                        <a href="#" class="nav-link" @click.prevent="currentPage = 1, 
                        paginateEntries()">First</a>
                    </li>
                    <li :class="['nav-item', {'disabled': currentPage === 1}]">
                        <a href="#" class="nav-link" @click.prevent="(currentPage < 1) ?
                         currentPage = 1 : currentPage -= 1,paginateEntries()">Prev</a>
                    </li>
                    ...
                    {'ellipsis': pagi === '...', 'active': pagi === currentPage }]">
                     ...
                     <li :class="['nav-item', {'disabled': currentPage === allPages}]">
                        <a href="#" class="nav-link" @click.prevent="(currentPage > allPages) ?
                         currentPage = allPages : currentPage += 1,paginateEntries()">Next</a>
                    </li>
                    <li :class="['nav-item', {'disabled': currentPage === allPages}]">
                        <a href="#" class="nav-link" @click.prevent="currentPage = allPages,
                        paginateEntries()">Last</a>
                    </li>
                </ul>
            </div>
        </div>
    ...
    methods: {
       ...
       filterByColumn() {
           ...
           if(Object.entries(filterCol).length >= 1) {
               filterData = $array.filtered(...Object.values(filterCol))(this.getCurrentEntries(), 
